...
// ========== TUGAS 2: TESTING ACCESSOR & SCOPE ==========

Route::get('/test-accessor-scope', function () {
    // Tambahkan style Bootstrap CDN agar tampilan badge terlihat rapi dan berwarna
    $html = '<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">';
    $html .= '<div class="container mt-4">';
    $html .= '<h1 class="mb-4 text-primary">Testing Accessor & Scope - Tugas 2</h1>';
    
    // 1. Buku dengan status_stok_badge & tahun_label
    $html .= '<h3>1. Semua Buku (Accessor Badge Stok & Label Tahun)</h3>';
    ...
    foreach (Buku::all() as $buku) {
        $html .= "<tr>
                    ...
                    <td><span class='badge bg-dark'>{$buku->tahun_label}</span></td>
                  </tr>";
    }
    $html .= '</tbody></table>';

    // 2. Buku Terbaru (Scope Terbaru)
    $bukuTerbaru = Buku::terbaru()->get();
    $html .= '<h3>2. Buku Terbaru (Scope Terbaru >= 2024): ' . $bukuTerbaru->count() . '</h3><ul>';
    foreach ($bukuTerbaru as $buku) {
        $html .= "<li>{$buku->judul} ({$buku->tahun_terbit})</li>";
    }
    $html .= '</ul><br>';

    // 3. Buku Stok Menipis (Scope StokMenipis)
    $stokMenipis = Buku::stokMenipis()->get();
    $html .= '<h3>3. Buku Stok Menipis (Scope StokMenipis < 5): ' . $stokMenipis->count() . '</h3><ul>';
    ...
    $html .= '<h3>4. Semua Anggota (Accessor Status Badge & Kategori Usia)</h3>';
    $html .= '<table class="table table-bordered table-striped mb-4">
                <thead class="table-dark">
    ...
    foreach (Anggota::all() as $anggota) {
        $html .= "<tr>
                    ...
                    <td><span class='badge bg-primary'>{$anggota->kategori_usia}</span></td>
                    <td>{$anggota->status_badge}</td>
                  </tr>";
    }
    $html .= '</tbody></table>';

    // 5. Anggota Terdaftar Bulan Ini (Scope)
    $bulanIni = Anggota::terdaftarBulanIni()->get();
    $html .= '<h3>5. Anggota Terdaftar Bulan Ini (Scope TerdaftarBulanIni): ' . $bulanIni->count() . '</h3><ul>';
    foreach ($bulanIni as $anggota) {
        $html .= "<li>{$anggota->nama} (Terdaftar: " . $anggota->tanggal_daftar->format('d-m-Y') . ")</li>";
    }
    $html .= '</ul>';
